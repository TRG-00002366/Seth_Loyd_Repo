...
df.withColumn("Salary_tier", 
                when(col("Salary")<= 60000.0, "Entry")
                .when(col("Salary") <= 75000.0, "Mid")
                .otherwise("Senior")
                ).show()
...
df.orderBy(col("department").desc()).show()
